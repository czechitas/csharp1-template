...
internal class C31_Async_Method_Return
{
    ...
    public async Task MyAsyncMethod(int myParameter)
    {
        //...
    }
    ...
    public async Task UsingAsyncMethods()
    {
        Task<string> myTask = GetHtml("http://...");
        await myTask;
        string html = myTask.Result;
    }

    // But normally it is easier and more readable to just access it implicitly in-line via await:
    public async Task AcessingResultInLine()
    {
        string html = await GetHtml("http://..."); //this however brings no time savings of asynchronicity
    }
    ...
    //TRYTHIS// public async Task<bool> TryGetHtml(string url, out string html) { return true; } // error
    ...
    public async Task<(bool, string)> TryGetHtml(string url) // Do this
    {
        if (string.IsNullOrEmpty(url))
        {
            return (false, "");
